...
let getAuthToken: AuthTokenGetter = () => authStorage.getToken();

/**
 * Allows the auth layer to provide a custom token getter if needed.
 */
...
function toApiError(error: AxiosError<Partial<ApiErrorPayload>>): ApiError {
  if (!error.response) {
    ...
    // offline, DNS failure, timeout, aborted, etc.
  ...
  if (
    ...
    message = details
      .map((detail) => detail.msg || "Validation error")
      .join(". ");
  } else if (
    data &&
    typeof data === "object" &&
 ...
 *
 * Executes an Axios request and returns only response.data.
 */
